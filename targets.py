...
SINE_SHIFT_OFFSET = ['SINE_SHIFT_OFFSET', lambda x: np.sin(3 * x + 2)]
BASIC_DIVISION = ['BASIC_DIVISION', lambda x: (x**2 + x)/(x+2)]
BASIC_INEQUALITY = ['BASIC_INEQUALITY', lambda x: (x <= 0) * np.sin(2 * x) + np.sin(3 * x) * (x > 0)]
BASIC_INEQUALITY = ['BASIC_INEQUALITY', lambda x: (x <= 0) * x + 3 * x * (x > 0)]
...
# IMPLICIT
CIRCLE = ['CIRCLE', lambda x: (1.0 - 2.0 * float(np.random.randint(0, 1))) * np.sqrt(1.0 - x ** 2)]
HYPERBOLA = ['HYPERBOLA', lambda x: 1.0 / x]

# ===========================================
### PHYSICS
# ===========================================
GRAVITATION = ['GRAVITATION', lambda x: (x[:,0] * x[:,1]) / x[:,2]** 2]
JACKSON  = ['JACKSON', lambda x: (x[:,0] ** 2 * (1 + x[:,0]) / (x[:,1] ** 5)).unsqueeze(-1)]
ELIPSE = ['ELIPSE', lambda x: ((x[:,0]**2) / 2+ (x[:,1] + 1) ** 2).unsqueeze(-1)]
...
def multi_ineq(x):
    f1 = ((x[:, 1] <= 0) * x[:, 0] + (x[:, 1]) ** 2 * (x[:, 1] > 0)).unsqueeze(-1)
    f2 = ((x[:, 0] <= 2) * (-x[:, 1]) + (x[:, 1]) * (x[:, 0] > 2)).unsqueeze(-1)
...
MULTI_INEQUALITY = ['MULTI_INEQUALITY', multi_ineq]
...
TARGET_SET = [IDENTITY, CONSTANT_MULTIPLY, SQUARE, SINE, BASIC_INEQUALITY, MULTI_INEQUALITY, PROTOTYPING,
              ...
              CONSTANT_INCREASE, CONSTANT_BUILDING, CIRCLE, HYPERBOLA, BASIC_DIVISION,
